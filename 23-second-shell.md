...
但 GitHub API 在某些网络环境下不通（我们这边是「分应用代理」：用户手机上只有名单里的 app 走代理，新装的 app 不在名单里，直连第三方境外域名很可能被 RST）。所以**让服务器代问**，app 只跟自己的域名说话——反正它已经要连那个域名了。

> 顺带一个把我们绕了 25 天的教训。第一次做壳时，app 打不开自家域名，我们定的结论是「新 app 不在代理名单里，直连被墙」，方案随即放弃。这次的壳**没加名单，一次就通**——那条「真因」是错的。回头查，当年为排查而设的「关闭 HTTP/3」（Caddy 全局 `protocols h1 h2`）一直留着没撤，很可能它才是真正治好问题的那一下。
>
> **一个在被放弃的方案上定下的结论，从此再也不会被复测。** 它随方案一起进冷宫，却以「已知事实」的形式留在文档和记忆里，下次被当成前提直接用。放弃方案时，要么把结论一并降级成「当时的猜测」，要么留一条最小复测路径。
