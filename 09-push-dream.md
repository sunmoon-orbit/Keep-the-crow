...
功能变多后，「AI 可以主动找我」会拆成几种不同动作：普通消息、来电、独处时间、自动发圈、外部平台唤醒等。一个总开关无法表达「可以发消息，但今天别打电话」。

有两种合理路线：

| 路线 | 适合 | 做法 |
|---|---|---|
| 一个总闸 | 功能少，用户只需要「安静/不安静」 | 关闭后撤销全部待发任务 |
| 总闸 + 分项权限 | 消息、电话、自动内容需分开控制 | 所有入口先查总闸，再查自己的分项闸 |

只在「创建任务」时检查权限不够。一次主动联系可能经过排队、生成、延迟和发送；用户在中间关了开关，旧任务不应继续溜出去。每个阶段都要重查，关闭时同时撤销队列中的待办。

还有一个常被忘记的边界：**外部唤醒源不是用户本人**。即使它能调用同一套工具，也不应继承所有主动联系权限。按来源加 allowlist，并让写操作有独立的授权闸。

验证不是只看 UI 开关变灰：在消息已进队、正在生成、等待延迟三种时机分别关闭，确认最终都没有外发。
