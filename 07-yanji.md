...
搜索结果顶部显示**真实总数**（例如“共 15 条”），并提供“近期优先 / 久远优先”切换。服务端响应最好同时返回 `items` 和 `total`；前端最多取 100 条用于选择，但不能拿 `items.length` 冒充总命中数。为了兼容旧接口，可以在没有 `total` 时退回 `items.length`，达到上限则显示 `100+`。

排序不要依赖接口碰巧返回的顺序。前端统一解析 `created_at` 或 `conv_date`：默认按时间降序，切换后升序；缺日期的结果稳定放在末尾。这样“最近在别的窗口聊过什么”和“最早什么时候提过这件事”都能一键找到。
...
纯前端也能通过 Streamable HTTP 连接远程 MCP，但不要把“能连接”误当成“可以无条件执行”。我们最后保留了五层限制：

1. 远程地址只允许 HTTPS；只有 `localhost` / `127.0.0.1` / `::1` 可用 HTTP。
2. URL 禁止内嵌用户名或密码，Bearer token 单独存放。
3. 新发现的工具默认关闭，用户逐个启用。
4. 标注 `readOnlyHint: true` 的工具可直接使用；其他工具还要给该服务单独打开“允许写操作”。
5. 一轮最多暴露 8 个外部工具，避免工具定义吃掉过多上下文。

SDK 用动态 `import()` 懒加载：用户没配置 MCP 时，不把一整套客户端库塞进聊天首屏。连接、列工具、调用分别设超时；CORS、401 和普通网络错误转成能看懂的提示，不能再用空 `catch` 吃掉。

还有一个容易忽略的边界：浏览器直连意味着 token 仍在用户设备中，且目标服务必须允许浏览器跨域。这适合个人前端和可信服务；面向多用户时应改成自己的后端代理，并在后端做鉴权、审计和网络出口限制。
...
从公开项目 chatnest（像素级仿 Claude 官方 App，非商业使用许可）借鉴的三个细节，成本都很低。若要复制其实现，先确认你的用途符合上游条款：
