...
## 渡口 / Agent：统一聊天手感，保留 Agent 信息层

渡口里的 Agent 聊天已经和 Murmur 共用一套消息表现：头像、消息气泡、Markdown、图片、发送时间、语音条与小屏布局保持一致。进入 Agent 页面后，阅读和操作方式与 Murmur 相同，不需要重新适应另一套聊天界面。

统一的是聊天手感，不是把两个页面强行做成完全相同。Agent 仍单独显示自己的模型、thinking 状态、额度和工具状态；这些属于运行信息，继续使用独立样式，避免和普通消息混在一起。
...
以下入口只在 Murmur 页面支持，工具卡中要明确标注“仅 Murmur 页面支持”，不要让用户点进去后才发现不可用：
...
这张分发表既是 UI 说明也是回归清单：新增工具时要分别检查 Murmur 和 Agent 的入口、执行能力与不可用提示，不能只验证某一个页面。

## 通话页的小屏、键盘与多窗口适配

通话页不能只按一台竖屏手机写死高度。页面高度以 `visualViewport` 和 `100dvh` 为准，并叠加 `safe-area`；软键盘弹出时重新计算可用区域。横屏、分屏和窄屏尺寸用 `clamp()` 收束头像、间距与字号，避免头像或操作区被挤出视口。

长字幕只在字幕区域内部滚动，不带着整个通话页跳动。小乌鸦、单头像、双头像三种形态使用同一套响应式边界，至少要检查：
...
安装 Android 原生壳后，页面不在前台时也可以收到来电：系统显示 heads-up 来电通知，并提供“接听 / 挂断”Action；锁屏或息屏时可通过 full-screen 来电界面处理。

用户侧需要同时打开两处开关：

1. 在 Android 系统设置里允许应用通知；若系统单独提供全屏通知、锁屏显示或悬浮通知权限，也要允许。
2. 在应用内打开“主动电话”开关。

任一处关闭，后台来电都可能只留下普通消息或完全不弹。页面前台来电仍沿用原有界面；原生通知负责补上后台、锁屏与息屏场景。因此上文“只有页面开着时才能响”的描述只适用于未安装或未授权原生来电能力的环境。

## 全局音乐播放器与手动点歌

音乐播放提升为全局状态：切换 Murmur、Agent 或其他页面时不断歌。底层只保留一个单例 `Audio`，避免页面各自创建播放器后同时出声。MiniPlayer 提供当前歌曲和基础控制，展开播放器提供队列、上一首、下一首、播放 / 暂停、进度与歌词；Media Session 把歌曲信息和常用控制同步到 Android 通知栏与锁屏。

用户可以在音乐入口搜索歌曲，并选择“立即播放”或“加入队列”。搜索走 GD Studio 链路；网易云没有结果时，依次回退酷我、Joox、咪咕。回退只改变音源，不改变用户看到的点歌流程。

“点给 TA”表示用户主动把一首歌分享给模型：

- 点歌时本机同时开始播放。
- Murmur 和 Agent 都支持。
- 模型收到的是歌曲元数据与歌曲卡片，可以据此回应。
- 设备正在播放的音频流不会上传给模型。

这一区分要写清楚：分享的是“这首歌是什么”，不是把扬声器里的声音转发给 AI。

## MiniPlayer 的移动端让位规则

移动端 MiniPlayer 固定在底部导航和 `safe-area` 上方；Murmur 与 Agent 的输入区根据播放器高度自动增加底部留白，不能互相遮挡。

软键盘打开时暂时隐藏 MiniPlayer 栏，让输入框和候选区获得完整空间，但单例 `Audio` 继续播放。键盘收起后播放器恢复，歌曲、进度和队列不重置。
